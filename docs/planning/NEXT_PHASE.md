...
1. 建立第三月 `month_03_plan.json`，先覆盖 Day 62–75 的第一条 10–15 分钟短循环；八个 Day 61 入口应归并为 2–3 个可维护玩法族，而不是复制八套内容。
2. 为第三月第一个循环增加一种真正的资源决策，例如补给、神圣力、体力或村务支持的取舍，让路线不只改变文本和关系。
3. 在日志中增加“当前承诺 / 当前紧张点”的可见摘要，帮助玩家理解 AI 记忆和关系回响，但不暴露永久记忆正文或控制 flags。
4. 继续准备真人盲测；只有真实玩家回填后才调整 Day 1–3 引导，保持 `pending-human-run`。
5. Underworld AI 仍只做候选对白、意图推荐和记忆候选；不接管日期、资源、flags、奖励或存档，不读取 `ai-shop`，不调用真实 Provider。

下一阶段完成定义：Day 62–75 至少形成一个资源投入 → 场景行动 → NPC 回响 → 剧情结果闭环；不同 Day 61 入口有可见差异但共享维护结构，并通过全量质量门和 GitHub 推送。
