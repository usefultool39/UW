...
本分支目标是把项目从“能走动的地图演示”推进到“可以逐步扩展的单人 RPG 纵切”。当前仍然是第一章原型，但已经把后续做梦幻西游式日常循环、巫师三式场景探索、以及 边境世界 风格 NPC 记忆互动需要的骨架铺开。
...
- NPC 记忆接入玩法：活动结果可以写入 艾琳/尤里 的重要记忆，之后 NPC 资料和对话上下文会读到这些记忆。
...
1. 先把露茵村第一章做深：补足读书、砍树、送午餐、夜晚回家、边界异常调查这些循环，让玩家每天都有明确选择。
...
```json
{
  ...
  "result_text": "你把草叶收进布袋，尤里 认出其中几种能缓解疲劳。",
  "time_cost": 1,
  "repeat": "daily",
...
- 给 艾琳/尤里 的 NPC profile 增加“最近承诺/最近紧张点”的 UI 摘要。
